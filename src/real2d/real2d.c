...
int main(int argc, char **argv) {
   ...
   fprintf(out, "  Parameters of trap: RC = %.4f, VXY = %.4f\n", rc, vxy);
   fprintf(out, "  Parameters of oscillation: AC = %.4f, OMEGA = %.4f\n", ac, omg);
   fprintf(out, "  Parameters of oscillation: kf = %.4f\n", kf);
   ...
   if(Nrun != 0){
      ...
      for(cntk = 0; cntk < Nrun; cntk ++) {

         as = a0 + ac * sin(omg * cntk * dt);
         gt_3d = 4. * pi * as * N0 * BOHR_RADIUS / aho;
         gt_2d = gt_3d / (sqrt(2. * (4. * atan(1.))) * d_z);
         G = gt_2d * par;
         ...
         if((dynaout != NULL) && (cntk % outstpt == 0)) {
            ...
            calcmax(&psi2, psi);
            calcmuen(&ek, &ev, &ei, psi, dpsix, dpsiy, tmpxi, tmpyi, tmpxj, tmpyj, tmpxk, tmpyk, tmpxl, tmpyl);
            calcakf(&akf, psi, abc, fftin, fftout, pf);
            fprintf(dyna, "%8le   %8le   %8le   %8le   %8le   %8le   %8le   %8le   %8le   %8le\n", (cntk + Npas) * dt * par, psi2, rms[1], rms[2], rms[3], rms[4], ek / par, ev / par, ei / par, akf);
            fflush(dyna);
         }
         if((tempout != NULL) && (cntk  % outdenstp == 0)){
            sprintf(filename, "%s_%li.txt", tempout, (long)(cntk * dt * par));
...
void readpar(void) {
   ...
   if((cfg_tmp = cfg_read("G_2D")) == NULL) {
      ...
      if((cfg_tmp = cfg_read("A0")) == NULL) {
         fprintf(stderr, "A0 is not defined in the configuration file.\n");
         exit(EXIT_FAILURE);
      }
      a0 = atof(cfg_tmp);

      g0_3d = 4. * (4. * atan(1.)) * a0 * N0 * BOHR_RADIUS / aho;
   ...
   if((cfg_tmp = cfg_read("RC")) == NULL) {
      fprintf(stderr, "RC is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   rc = atof(cfg_tmp);

   if((cfg_tmp = cfg_read("VXY")) == NULL) {
      fprintf(stderr, "VXY is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   vxy = atof(cfg_tmp);

   if((cfg_tmp = cfg_read("AC")) == NULL) {
      fprintf(stderr, "AC is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   ac = atof(cfg_tmp);

   if((cfg_tmp = cfg_read("OMEGA")) == NULL) {
      fprintf(stderr, "OMEGA is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   omg = atof(cfg_tmp);
 ...
 */
   ...
   for(cnti = 0; cnti < Nx; cnti ++) {
      ...
      kx[cnti] = (cnti - Nx2) * 2 * pi / (Nx * dx);
      kx2[cnti] = kx[cnti] * kx[cnti];
      for(cntj = 0; cntj < Ny; cntj ++) {
         y[cntj] = (cntj - Ny2) * dy;
         y2[cntj] = y[cntj] * y[cntj];
         ky[cntj] = (cntj - Ny2) * 2 * pi / (Ny * dy);
         ky2[cntj] = ky[cntj] * ky[cntj];
         if (x2[cnti] + y2[cntj] <= rc2){
            pot[cnti][cntj] = 0.;
         }
         else if (x2[cnti] + y2[cntj] <= ro2){
            pot[cnti][cntj] = vxy;
         }
         else{
            pot[cnti][cntj] = 0.;
...
//            srand((unsigned int)time(NULL));
            psir = randn(0, 0.1 * sqrt(N0) * tmp)/sqrt(N0);
            psii = randn(0, 0.1 * sqrt(N0) * tmp)/sqrt(N0);
         }
...
void calcakf(double *akf, double complex **psi, double **abc, fftw_complex *fftin, fftw_complex *fftout, fftw_plan p){
   long cnti, cntj;
   double psi2, tmp;
   ...
   for(cnti = 0; cnti < Nx; cnti ++) {
      for(cntj = 0; cntj < Ny; cntj ++) {
         psi2 = cabs(psi[cnti][cntj]);
         psi2 *= psi2;
         fftin[cnti * Ny + cntj][0] = psi2;
         fftin[cnti * Ny + cntj][1] = 0;
      }
   }
   ...
   for(cnti = 0; cnti < Nx; cnti++){
      for(cntj = 0; cntj < Ny; cntj++){
         tmp = fftout[cnti * Ny + cntj][0] + I * fftout[cnti * Ny + cntj][1];
         abc[cnti][cntj] = cabs(tmp) / sqrt(Nx * Ny);
      }
   }
   ...
   *akf = 0.;
   for(cnti = 0; cnti < Nx; cnti ++) {
      for(cntj = 0; cntj < Ny; cntj ++) {
         if(cabs(kx2[cnti] + ky2[cntj] - kf2) < 0.005){
            *akf += abc[cnti][cntj];
         }
      }
   }
